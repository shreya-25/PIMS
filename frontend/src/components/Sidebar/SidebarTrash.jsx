...
// helper to group by case
const groupCountByCase = (arr, statusKey = "In Review") =>
  (arr || []).reduce((acc, l) => {
    const caseNo = String(l.caseNo ?? "");
    if (!caseNo) return acc;
    if (l.leadStatus === statusKey) {
      acc[caseNo] = (acc[caseNo] || 0) + 1;
    }
    return acc;
  }, {});

// count map depends on role
const leadCountByCase = useMemo(() => {
  if (signedInRole === "Investigator") {
    // you already fetched assignedLeadsList (status === "Assigned")
    // make a per-case count for Assigned
  ...
  // CM / DS (or default): prefer allLeads if provided; else pendingLeadReturns if it represents "In Review"
  ...
  return {};
}, [signedInRole, assignedLeadsList, allLeads, pendingLeadReturns]);
        ...
        {caseDropdownOpen && (
          <ul className="dropdown-list1">
            {caseList
              ...
              .map((c) => {
                const count = leadCountByCase[String(c.id)] != null ? leadCountByCase[String(c.id)]: 0;
