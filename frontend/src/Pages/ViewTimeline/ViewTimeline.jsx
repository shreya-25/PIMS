...
//     // Check if the string contains AM/PM; if not, assume it's already 24-hr.
//     if (!time12h || !time12h.toUpperCase().includes('AM') && !time12h.toUpperCase().includes('PM')) {
//       return time12h;
//     }
...
//     // Filter by date range using the entry.date property (adjust if needed)
...
//  // Filter by time range using eventStartTime and eventEndTime
//  if (filters.startTime && filters.endTime) {
//   // Convert filter times from possible 12-hour format to 24-hour format
//   const filterStart24 = convert12To24(filters.startTime);
//   const filterEnd24 = convert12To24(filters.endTime);

//   filtered = filtered.filter((entry) => {
//     // Ensure the entry has both start and end times
//     if (!entry.eventStartTime || !entry.eventEndTime) return false;
    
//     // Convert entry times; assume they are already in a proper format 
//     // If your API returns them in 12-hour format, you might call convert12To24 on those too.
//     const entryStart24 = convert12To24(entry.eventStartTime);
//     const entryEnd24 = convert12To24(entry.eventEndTime);

//     // Create Date objects on a dummy date for comparison
...
//     // Overlap logic: include events that overlap the filter window
          ...
          <ul className="sidebar-list">
            ...
            {caseDropdownOpen && (
              <ul className="dropdown-list1">
                ...
                <li className="sidebar-item" onClick={() => navigate("/LeadsDesk", { state: { caseDetails } })}>
                  View Leads Desk
                </li>
                <li className="sidebar-item" onClick={() => navigate("/HomePage", { state: { caseDetails } })}>
                  Go to Home Page
                </li>
              </ul>
            )}
