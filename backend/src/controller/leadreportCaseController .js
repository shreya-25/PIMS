...
async function buildLeadBuffer(lead, { includeAll, characterOfCase, userMap, shouldWatermark, pageHeader = null, timezone }) {
  ...
  // ── Optional blue page header (used for first lead when no exec summary cover page) ──
  if (pageHeader) {
    const { user, reportTimestamp, caseNo, caseName, reportTitle } = pageHeader;
    const headerHeight = 80;
    doc.rect(0, 0, doc.page.width, headerHeight).fill("#003366");
...
async function buildCaseReportBufferPerLead({
  ...
  // 1. Cover page (header + exec summary) — only when exec summary exists;
  //    otherwise skip the blank cover and go straight to lead details.
  let finalBuffer = showExecSummary
    ? await buildHeaderBuffer({
        user,
        reportTimestamp,
        caseNo,
        caseName,
  ...
  if (sortedLeads.length === 0) {
    ...
    emptyDoc.end();
    await ee;
    const emptyBuf = Buffer.concat(ec);
    return finalBuffer ? mergeWithAnotherPDF(finalBuffer, emptyBuf) : emptyBuf;
  }

  // 2. For each lead: lead mini-PDF immediately followed by that lead's PDFs
  let isFirstLead = true;
  for (const lead of sortedLeads) {
    // When there's no exec summary cover page, embed the blue header into the first lead's page
    const pageHeader = (!showExecSummary && isFirstLead)
      ? { user, reportTimestamp, caseNo, caseName, reportTitle }
      : null;
    isFirstLead = false;
...
async function generateCaseReport(req, res) {
  ...
  let characterOfCase = "";
  const resolvedCaseId = caseIdParam || String(leadsData?.[0]?.caseId || "");
  if (resolvedCaseId) {
    const caseDoc = await Case.findById(resolvedCaseId).select("characterOfCase").lean();
  ...
  try {
    ...
    // Generated by — rendered FIRST (cursor is near top) so explicit y values
    // move forward, guaranteeing the text lands inside the blue bar.
    const rightBlockX = doc.page.width - 210;
    const rightBlockW = 200;
    doc.fillColor("white").font("Helvetica").fontSize(8);
    doc.text(`Generated by: ${user}`, rightBlockX, headerHeight - 24, {
      width: rightBlockW, align: "right"
    });
    doc.text(reportTimestamp, rightBlockX, headerHeight - 12, {
      width: rightBlockW, align: "right"
    });

    // Logo — drawn after text so the image appears on top of any overlap
    const logoHeight = 70;
    const logoY = (headerHeight - logoHeight) / 2;   // = 5
    const logoPath = path.join(__dirname, "../assets/newpolicelogo.png");
    if (fs.existsSync(logoPath)) {
      doc.image(logoPath, 10, logoY, { width: 70, height: 70 });
    }

    // Title block — both lines share the same x/width so they share the same center.
    // Start just after the logo (x=85) so even long case names fit on one line.
    const titleX  = 180;
    const titleW  = rightBlockX - titleX - 10;
    const caseLabel = `Case: ${caseNo}: ${caseName}`;
    const lineGap = 4;
    ...
    const titleH = doc.heightOfString(reportTitle, { width: titleW });
    doc.font("Helvetica").fontSize(10);
    const caseH  = doc.currentLineHeight(true);  // single line — no wrap

    const blockH      = titleH + lineGap + caseH;
    const titleStartY = Math.round((headerHeight - blockH) / 2);

    // Center the case label at the same midpoint as the title, then place it in
    // one line regardless of how long the text is.
    const titleCenter  = titleX + titleW / 2;
    const caseLabelW   = doc.widthOfString(caseLabel);
    const caseLabelX   = Math.max(85, titleCenter - caseLabelW / 2);
...
async function generateTimelineOnlyReport(req, res) {
  try {
    ...
    const fmtDateTimeCell = (e) => {
      const startDate = fmtDate(e.eventStartDate || e.eventDate);
      const endDate   = fmtDate(e.eventEndDate);
      const startTime = fmtTime(e.eventStartTime);
      const endTime   = fmtTime(e.eventEndTime);

      let dateLine = startDate;
      if (endDate && endDate !== startDate) dateLine += ` – ${endDate}`;

      let timeLine = "";
      if (startTime) {
        timeLine = startTime;
        if (endTime && endTime !== startTime) timeLine += ` – ${endTime}`;
      }

      return timeLine ? `${dateLine}\n${timeLine}` : dateLine;
    };

    // ── Build PDF (margin: 50 matches all other case reports) ─────────────
    const doc = new PDFDocument({ size: "LETTER", margin: 50 });
    ...
    // ── Blue banner header — identical pattern to buildHeaderBuffer ─────────
    const HEADER_H = 80;
    doc.rect(0, 0, pageW, HEADER_H).fill("#003366");

    const rightBlockX = pageW - 210;
    const rightBlockW = 200;
    doc.fillColor("white").font("Helvetica").fontSize(8);
    doc.text(`Generated by: ${user}`, rightBlockX, HEADER_H - 24, { width: rightBlockW, align: "right" });
    doc.text(timestamp,              rightBlockX, HEADER_H - 12, { width: rightBlockW, align: "right" });

    const logoPath = path.join(__dirname, "../assets/newpolicelogo.png");
    if (fs.existsSync(logoPath)) {
      doc.image(logoPath, 10, (HEADER_H - 70) / 2, { width: 70, height: 70 });
    }
    ...
    const caseH      = doc.currentLineHeight(true);
    const blockH     = titleH + lineGap + caseH;
    const titleStartY = Math.round((HEADER_H - blockH) / 2);
    ...
    // ── Table layout constants (matches drawStructuredLeadDetails style) ──────
    const COL_DATE = 130;
    const COL_LEAD = 50;
    const COL_DESC = tableWidth - COL_DATE - COL_LEAD;

    const PAD    = 5;   // cell padding (matches existing reports)
    const HDR_H  = 20;  // fixed header row height (matches existing reports)

    // ── Empty state ────────────────────────────────────────────────────────
    if (entries.length === 0) {
      doc.font("Helvetica").fontSize(11).fillColor("#555")
        .text("No timeline entries found for this case.", MARGIN, currentY, { width: tableWidth });
      doc.end();
      await docEnd;
    ...
    // ── Table column-header row (redrawn on every page) ────────────────────
    const drawColHeader = (y) => {
      const cols = [
        { label: "Event Date & Time", w: COL_DATE },
        { label: "Lead #",            w: COL_LEAD },
        { label: "Description",       w: COL_DESC },
      ];
      let cx = MARGIN;
      for (const col of cols) {
        doc.rect(cx, y, col.w, HDR_H).fillAndStroke("#e0e0e0", "#bbb");
        doc.fillColor("#000").font("Helvetica-Bold").fontSize(11)
          .text(col.label, cx + PAD, y + PAD, { width: col.w - PAD * 2, lineBreak: false });
        cx += col.w;
      }
      return y + HDR_H;
    };

    const calcRowH = (entry) => {
      const dtText   = fmtDateTimeCell(entry);
      const descText = (entry.eventDescription || "").trim();
      const hDate = doc.font("Helvetica").fontSize(10)
        .heightOfString(dtText,   { width: COL_DATE - PAD * 2 });
      const hLead = doc.font("Helvetica").fontSize(10)
        .heightOfString(String(entry.leadNo ?? ""), { width: COL_LEAD - PAD * 2 });
      const hDesc = doc.font("Helvetica").fontSize(10)
        .heightOfString(descText, { width: COL_DESC - PAD * 2 });
      return Math.max(20, hDate, hLead, hDesc) + PAD * 2;
    };

    const drawRow = (entry, y) => {
      const rowH     = calcRowH(entry);
      const dtText   = fmtDateTimeCell(entry);
      const leadText = String(entry.leadNo ?? "");
      const descText = (entry.eventDescription || "").trim();

      // Date cell
      doc.rect(MARGIN, y, COL_DATE, rowH).stroke();
      doc.font("Helvetica").fontSize(10).fillColor("#000")
        .text(dtText, MARGIN + PAD, y + PAD, { width: COL_DATE - PAD * 2, lineBreak: true });

      // Lead # cell
      doc.rect(MARGIN + COL_DATE, y, COL_LEAD, rowH).stroke();
      doc.font("Helvetica").fontSize(10).fillColor("#000")
        .text(leadText, MARGIN + COL_DATE + PAD, y + PAD,
          { width: COL_LEAD - PAD * 2, align: "center", lineBreak: false });

      // Description cell
      doc.rect(MARGIN + COL_DATE + COL_LEAD, y, COL_DESC, rowH).stroke();
      doc.font("Helvetica").fontSize(10).fillColor("#000")
        .text(descText, MARGIN + COL_DATE + COL_LEAD + PAD, y + PAD,
          { width: COL_DESC - PAD * 2, lineBreak: true });

      return rowH;
    };

    // ── Render table ───────────────────────────────────────────────────────
    currentY = drawColHeader(currentY);
    ...
    for (const entry of entries) {
      ...
      currentY += drawRow(entry, currentY);
    }
