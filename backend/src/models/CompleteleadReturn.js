...
// Compound index to ensure unique version numbers per lead
completeLeadReturnSchema.index({ leadNo: 1, versionId: 1 }, { unique: true });

// Index to quickly find current version
completeLeadReturnSchema.index({ leadNo: 1, isCurrentVersion: 1 });

// Static method to get the latest version for a lead
completeLeadReturnSchema.statics.getCurrentVersion = async function(leadNo) {
    return this.findOne({ leadNo, isCurrentVersion: true }).sort({ versionId: -1 });
};

// Static method to get all versions for a lead
completeLeadReturnSchema.statics.getAllVersions = async function(leadNo) {
    return this.find({ leadNo }).sort({ versionId: -1 });
};

// Static method to get a specific version
completeLeadReturnSchema.statics.getVersion = async function(leadNo, versionId) {
    return this.findOne({ leadNo, versionId });
};
